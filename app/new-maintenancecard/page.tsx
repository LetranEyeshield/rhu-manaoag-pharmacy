...
import { MaintenancecardType } from "../types/Maintenancecard";
import { useRouter } from "next/navigation";

export default function MaintenancecardForm() {
  ...
  const createMutation = useMutation({
    mutationFn: (input: MaintenancecardType) =>
      graphqlRequest(
        `
      mutation CreateMaintenancecard($input:MaintenancecardInput!){
        createMaintenancecard(input:$input){
          _id
          cardName
          cardDate
          initialStock
    ...
    onSuccess: () => {
      ...
      toast.success("New Maintenance Card Added Successfully", {
        duration: 3000,
        style: {
          padding: "4px",
          fontSize: "16px",
    ...
    onError: (error: any) => {
      const message = error.message || "Error creating maintenance card";
